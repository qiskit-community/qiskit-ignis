...
from qiskit.ignis.characterization.\
    pulse_library import (cr_designer_variable_duration,
                          var_duration, get_control_channels)
...
def cr_hamiltonian_tomography_circuits(qubit_pair: Tuple[int, int],
    ...
    """
    Generate `QuantumCircuit` to measure CR Hamiltonian.
    ...
    This function adds state preparation and measurement schedules
    to `cr_circ`. Returned circuit should be transpiled and scheduled
    to execute as a pulse job.
    Schedules should be executed with `meas_level=2`,
    or use discriminator to get counts.
    ...
    Returns:
        experiments to run, `xdata` for fitter.

    Example: ::
        import qiskit
        from qiskit.ignis.characterization.hamiltonian import *

        # replace this with your target backend
        backend = FakeOpenPulse2Q()

        params = {'cr_amp': 0.1, 'sigma': 2, 'risefall': 5}
        durations = [20, 30, 40]

        qc, maps, gates = create_cr_circuit((0, 1), backend, params)
        qcs, xs = cr_hamiltonian_tomography_circuits((0, 1), qc, durations, 1)

        qcs = qiskit.transpile(qcs, backend, basis_gates=gates)
        scheds = qiskit.schedule(qcs, backend, maps)

        qobj = qiskit.assemble(scheds, backend, meas_level=2)
    """
    circ_names = ['cr_ham_tomo_sched_%d' % ii
                  for ii in range(len(cr_durations))]
    ...
    for cind, cr_duration in enumerate(cr_durations):
        for c_state in (0, 1):
            ...
            for op in ('X', 'Y', 'Z'):
                qc_temp = qc.copy()
                qc_temp += pauli_measurement_circuit(op,
                                                     qreg[t_qubit],
                                                     creg[0])
...
def create_cr_circuit(qubit_pair: Tuple[int, int],
    ...
    """
    A helper function to generate circuit abstraction of CR
    with variable pulse duration. Gaussian flattop pulse parameter
    should be defined as a python dictionary to generate cross resonance
    and cancellation pulses from `gaussian_square` pulses.

    ::
        cross_resonance_params = {'cr_amp': 0.1, 'ca_amp': 0.1,
                                  'sigma': 5, 'risefall': 30}

    This function converts pulse sequence into `QuantumCircuit`
    and returns it with new pulse command definition and basis gates
    to be provided to the pulse scheduler and transpiler to
    revert the circuit into pulse schedule.

    Args:
        qubit_pair: index of control and target qubit.
        backend: backend object of target system.
        cross_resonance_params: python dictionary of cross resonance
            pulse arguments.
        use_echo: use echo sequence if `True`.
        circ_inst_map: mapper of circuit to instructions.
            use backend default if not provided.
