...
class GraphDecoder ():
    ...
    Attributes
    * `code`: Object describing a compatible form of code, as supplied when the decoder is initialized.
    * `S`: Graph describing connections between non-trivial syndrome elements.
    '''
    def __init__(self,code,S=None):
        '''
        Initializes the decoder for the supplied `code` by running `_make_syndrome_graph()`. Since this process can take some
        time, it is also possible to load in a premade `S`. However , if this was created for a differently defined `code`,
    ...
    def _make_syndrome_graph (self):
        '''
        This method injects all possible Pauli errors into the circuit for `code`. This is done by examining the qubits
        used in each gate of the circuit for a stored logical 0. A graph is then created with a node for each non-trivial
        syndrome element, and an edge between all such elements that can be created by the same error.
        '''
        ...
        job = execute( list(error_circuit.values()), Aer.get_backend('qasm_simulator') )
                    ...
                    for string in results:
                        ...
                        for syndrome_type, _ in enumerate(separated_string):
                            for syndrome_round in range(len(separated_string[syndrome_type])):
                                for syndrome_element in range(len(separated_string[syndrome_type][syndrome_round])):
                                    if separated_string[syndrome_type][syndrome_round][syndrome_element]=='1':
                                        nodes.append((syndrome_type,syndrome_round,syndrome_element))
    ...
    def make_error_graph (self,string,subgraphs=None):
        ...
        for syndrome_type, _ in enumerate(separated_string):
            for syndrome_round in range(len(separated_string[syndrome_type])):
                for syndrome_element in range(len(separated_string[syndrome_type][syndrome_round])):
                    if separated_string[syndrome_type][syndrome_round][syndrome_element]=='1' or syndrome_type==0:
                        for subgraph in subgraphs[syndrome_type]:
                            E[subgraph].add_node( (syndrome_type,syndrome_round,syndrome_element) )
        

        # for each pair of nodes in error create an edge and weight with the distance
        for subgraph in set_subgraphs:
            for source in E[subgraph]:
                for target in E[subgraph]:
                    if target!=(source):
                        distance = nx.shortest_path_length(self.S,source,target)
                        E[subgraph].add_edge(source,target,weight=-distance)
                    
        return E
    
    def matching (self,string,subgraphs=None):
        
        # this matching algorithm is designed for a single graph
        E = self.make_error_graph(string)['0']
        
        # set up graph that is like E, but each syndrome node is connected to a separate copy of the nearest logical node
        E_matching = nx.Graph()
        syndrome_nodes = []
        logical_nodes = []
        logical_neighbours = []
        ...
        for source in syndrome_nodes:
            for target in syndrome_nodes:
                if target!=(source):
                    E_matching.add_edge( source, target, weight=E[source][target]['weight'] )
            ...
            E_matching.add_edge( source, nearest_logical+source, weight=potential_logical[nearest_logical] )
        ...
        for log in results:
            ...
            if algorithm=='matching':
                for string in results[log]:
                    try:
                        corrected_results[self.matching(string)] += results[log][string]
                    except:
                        corrected_results[self.matching(string)]  = results[log][string]           
            else:
                    warnings.warn("The requested algorithm "+str(algorithm)+" is not known.", Warning)
