...
class quantum_volume:
	def __init__(self, backend, ntrials=50, qubit_list=None, noise_model=None, basis_gates=None):
		...
		if 'qasm' in backend.name():
			if noise_model == None:
				...
				raise QiskitError("Simulators need qubit lists.")
			else:
		...
		if 'fake' in backend.configuration().backend_name:
			self._is_simulator = True    
    
		self._ntrials = ntrials
		self._qlist_cond = False
		if self._qubit_list:
			self._max_depth = len(qubit_list)
			self._list_of_qubit_lists = self.generate_qubit_lists()
			self._qlist_cond = True
		else:
			self._max_depth = self._backend.configuration().num_qubits
	...
	def generate_list_of_trails(self,depth):
		if self._qlist_cond:
			y = [self._list_of_qubit_lists[depth - 3]]
			return circuits.qv_circuits_opt(y,
			 ntrials=self._ntrials, max_qubits=self._max_depth, backend=self._backend) #The -2 corrects the depth -> index
		else:
			return circuits.qv_circuits_opt(ntrials = self._ntrials, max_qubits=self._max_depth, backend=self._backend)

	def find_quantum_volume(self):
		managed_job_list = []
		sim_results_list = []
		if not self._qlist_cond:
			qv_circs_real, qv_circs_sim, self._list_of_qubit_lists = self.generate_list_of_trails(depth=None)
		for depth in range(3, self._max_depth + 1): #Don't check if depth == 1 or 2
			sim_results = []
			real_results = []
			if self._qlist_cond:
				qv_circs_real, qv_circs_sim = self.generate_list_of_trails(depth=depth) #Creates one list of circuits to run and one to simulate.
			...
			for trial in range(self._ntrials):
				if self._qlist_cond:
					...
					sim_results.append(qiskit.execute(qv_circs_sim[trial][depth-3], backend=self._simulator_backend).result())
			sim_results_list.append(sim_results)
			#Create the job manager for the depth run
			job_manager = IBMQJobManager()
			#Strips the outer list from qv_circs_real
			if self._qlist_cond:
				qv_circs_real_temp = [circ[0] for circ in qv_circs_real]
			else:
				qv_circs_real_temp = [circ[depth-3] for circ in qv_circs_real]
			init_layout = list(qv_circs_real_temp[0]._layout.get_physical_bits().keys())
			if self._is_simulator:
				managed_job = execute(qv_circs_real_temp, backend = self._backend ,basis_gates=self._basis_gates, initial_layout = init_layout)
			else:
				#Transpile circuits, and create a ManagedJobSet object to later call the results, then stores it in a list
				qv_circs_real = transpile(qv_circs_real_temp, backend = self._backend, initial_layout = init_layout)
				managed_job = job_manager.run(qv_circs_real_temp, backend = self._backend, initial_layout = init_layout)
	...
	def find_best_qv_real(self,managed_job_list,sim_results_list):
		...
		for managed_job_index in range(len(managed_job_list)):
			...
			success, qv_fitter  = self.qvolume(current_list,sim_results,repal_results)
	...
	def q_fitter(self):
		...
		real_results=[]
		if self._qubit_list:
			qv_circs_real, qv_circs_sim = qv.qv_circuits_opt(self.generate_qubit_lists(), ntrials=self._ntrials,backend=self._backend)     
		else:
			qv_circs_real, qv_circs_sim = qv.qv_circuits_opt(ntrials=self._ntrials, max_qubits = self._max_depth,backend=self._backend)
		for trial in range(self._ntrials):
			if trial%5==0: print(trial)            
			sim_results.append(qiskit.execute(qv_circs_sim[trial], backend=self._simulator_backend).result())
			if self._is_simulator and 'fake' not in self._backend.configuration().backend_name:
				...
				real_results.append(execute(qv_circs_real[trial], backend = self._backend,basis_gates=self._basis_gates).result())
