...
class NonCliffordRBFitter(RBFitterBase):
    ...
    def __init__(self, nonclifford_Z_result, nonclifford_X_result,
        ...
        self._rb_pattern = rb_pattern
        self._fit_nonclifford = []

        self._rbfit_Z = RBFitter(
            nonclifford_Z_result, elmnts_lengths, rb_pattern)
        self._rbfit_X = RBFitter(
            nonclifford_X_result, elmnts_lengths, rb_pattern)

        self.rbfit_Z.add_data(nonclifford_Z_result)
        self.rbfit_X.add_data(nonclifford_X_result)

        if not (nonclifford_Z_result is None and
                nonclifford_X_result is None):
            self.fit_data()

    @property
    def rbfit_Z(self):
        """Return the nonclifford Z fitter."""
        return self._rbfit_Z

    @property
    def rbfit_X(self):
        """Return the nonclifford X fitter."""
    ...
    @property
    def fit_nonclifford(self):
        """Return non-Clifford fit parameters"""
        return self._fit_nonclifford
    ...
    def add_data(self, new_nonclifford_Z_result,
                 new_nonclifford_X_result, rerun_fit=True):
        """
        Add a new result.
        ...
        Additional information:
            ...
            the output of circuits generated by randomized_becnhmarking_seq
        """
        self.rbfit_Z.add_data(new_nonclifford_Z_result, rerun_fit)
        self.rbfit_X.add_data(new_nonclifford_X_result, rerun_fit)
    ...
    def fit_data(self):
        ...
        for patt_ind, (_, qubits) in enumerate(zip(self._cliff_lengths,
            ...
            self._fit_nonclifford.append({'alpha': alpha,
                                          'alpha_err': alpha_err,
                                          'epg_est': epg_est,
    ...
    def plot_rb_data(self, pattern_index=0, ax=None,
                     add_label=True, show_plt=True):
        """
        Plot non-Clifford randomized benchmarking data of a single pattern.
        ...
        if add_label:
            ...
            ax.text(0.6, 0.9,
                    ...
                    (self._fit_nonclifford[pattern_index]['alpha'],
                     self._fit_nonclifford[pattern_index]['alpha_err'],
                     self._fit_nonclifford[pattern_index]['epg_est'],
                     self._fit_nonclifford[pattern_index]['epg_est_err']),
                    ha="center", va="center", size=14,
                    bbox=bbox_props, transform=ax.transAxes)
