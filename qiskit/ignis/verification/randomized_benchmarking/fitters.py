...
class CorrelatedRBFitter(RBFitterBase):
    ...
    def __init__(self, backend_result, cliff_lengths, rb_pattern):
        """
        Args:
            ...
            cliff_lengths (list): the Clifford lengths, 2D list i x j where i is the
                number of patterns, j is the number of cliffords lengths
            rb_pattern (list): the pattern for the rb sequences.
            will be added - max_weight (int): maximum weight to fit (-1 fit all)
        """
        ...
        for i in range(self._n_subsystems):
            ...
            for ii in range(self._n_partitions):
                tmplist = [self._subsystems[-1][ii]] * len(rb_pattern[ii])
        ...
        self._zdict_ops = []
        for subsys in self._subsystems2:
            self._zdict_ops.append(statedict.copy())
            for i in range(2**self._nq):
                # if the operator (expressed as an integer) and the state
                # overlap is even then the z operator is '1' otherwise it's -1
                if bin(i & subsys).count('1') % 2 != 0:
    ...
    @property
    def fit_alphas(self):
        """Return alphas, dict (subsystems in int representation as keys and alphas as values)."""
        alphadict = {}
    ...
    def fit_epsilon(self):
        """Return epsilons, dict (subsystems in int representation as keys list of
        [epsilon, estimation_error] as values)."""
    ...
    def add_data(self, new_backend_result, rerun_fit=True):
        ...
        Args:
            new_backend_result (list): list of rb results
            rerun_fit (bool): whether to re-caculate the means and fit the result
        Additional information:
            Assumes that 'new_backend_result' is the output of execution of circuits
            generated by randomized_becnhmarking_seq
        """
        if new_backend_result is None:
            return
        ...
        for result in new_backend_result:
            ...
            # update the number of seeds *if* new ones added.
            # Note, no checking if we've done all the cliffords
            for rbcirc in result.results:
                nseeds_circ = int(rbcirc.header.name.split('_')[-1])
                if nseeds_circ not in self._nseeds:
                    self._nseeds.append(nseeds_circ)
...
#                    "results")
    ...
    def calc_data(self):
        """
        Measure the correlators into an internal variable _raw_data, which is a 3-dimensional
        list, where item (i,j,k) is the probability to measure the "i" correlator for
        seed no. j and vector length self._cliff_lengths[i][k].
        Additional information:
            Assumes that 'self._result_list' is the output of execution of circuits
            generated by randomized_becnhmarking_seq
        """
        ...
        subsystems_n = len(self._subsystems)
        for sub_ind in range(subsystems_n):
    ...
    def calc_statistics(self):
        """
        Extract averages and std dev from the raw data (self._raw_data).
        Assumes that self._calc_data has been run. Output into internal _ydata variable:
            ydata is a list of dictionaries (length number of patterns).
            Dictionary ydata[i]:
            ydata[i]['mean'] is a numpy_array of length n;
                        entry j of this array contains the mean probability of
                        success over seeds, for vector length self._cliff_lengths[i][j].
            And ydata[i]['std'] is a numpy_array of length n;
                        entry j of this array contains the std of the probability of
                        success over seeds, for vector length self._cliff_lengths[i][j].
        """
    ...
    def fit_data_pattern(self, patt_ind, fit_guess):
        """
        Fit the RB results of a particular pattern to an exponential curve.
        For correlated RB this means fit for a particular subsystem
        Args:
            patt_ind (int): index of the subsystem to fit
            fit_guess (array_like): guess values for the fit (length 3)
        Puts the results into a list of fit dictionaries:
            where each dictionary corresponds to a pattern and has fields:
            'params' - three parameters of rb_fit_fun. The middle one is the exponent.
            'params_err' - the error limits of the parameters.
        """
    ...
    def fit_data(self):
        """
        Fit the RB results to an exponential curve, by fitting each of the
        subsystems (z correlators).
        Use the data to construct guess values for the fits.
        Puts the results into a list of fit dictionaries:
            where each dictionary corresponds to a pattern and has fields:
            'params' - three parameters of rb_fit_fun. The middle one is the exponent.
    ...
    def plot_rb_data(self, sub_index=0, ax=None,
        ...
        Args:
            sub_index (int): which RB subsystem to plot
    ...
    def _precalc_r_coeff(self):
        ...
        # precompute the matrix of coefficients for calculating the rauli's
    ...
    def _depol_Rauli(self, pauli_elem, depol_subsys, p):
        ...
        e.g. rho' = (1-e)*rho + e*II/d (II in the depol_subspace subspace only!)
        Args:
            pauli_elem (list): List of the pauli elements of the Rauli.
            This is a string of 1s and 0s which map to Z and I
            e.g. '101' -> Pauli ZIZ
            depol_subsys (list): The subsystem that is depolarizing (in the same
            format as pauli_elem)
            p (int): Depolarizing probability ([0,1])
        Returns:
            list: list of the rauli elements for pauli_elem
        Additional Info:
            The rauli's for a depolarizing map can only be 1 or 1-p
        """

        rauli = np.zeros(len(pauli_elem), dtype=float)
        for (state_index, state_paulis) in enumerate(pauli_elem):
            all_id = 1
            for j, state_pauli in enumerate(state_paulis):
                if depol_subsys[j] == '1' and state_pauli == '1':
                    all_id = 0
    ...
    def _calc_alphas(self, eps_list):
        ...
        Args:
            eps_list (list): list of epsilons from the error model (ordered
            using the self._subsystems list)
        Returns:
            list: List of alphas given eps_list
        """

        eps_len = len(eps_list)
        alpha_calc = np.ones(eps_len, dtype=float)

        for ii in range(eps_len):
            for jj in range(eps_len):
    ...
    def _alpha_diff_func(self, eps_list):
        ...
        Args:
            ...
            using the self._subsystems list)
        Returns:
            list: List of the differences between the calculated and measured alphas
        """
    ...
    def fit_alphas_to_epsilon(self, init_guess=None):
        ...
        Args:
            init_guess (array_like): starting point for the fit for each epsilon.
            If none will use 0.01 for each epsilon.
        Returns:
            int: cost value of the optimization
        """
    ...
    def plot_epsilon(self, ax=None):
        ...
        if nq < 6:
            # for small amount of qubits - use the same colors each time
            color_list = ['black', 'red', 'blue', 'green', 'yellow']
        else:
            # for larger amount of epsilon - sample colors from color map
            # the size of the color map is the number of qubits
            color_list = plt.cm.get_cmap("Set1", nq)
        prev_weight = 0
