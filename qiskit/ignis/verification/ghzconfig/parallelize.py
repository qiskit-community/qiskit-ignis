...
class BConfig:
    '''
    This class creates an object that creates a GHZ circuit with parallellized CNOT gates to increase fidelity
    '''
    
    def __init__(self, backend, nodes={},indicator=True):
        self.nodes = nodes
        self.backend = backend
        self.cmap = backend._configuration.coupling_map
        self.add_nodes()
    ...
    def cmap_calib(self):
        '''
        Only intended for public devices (doubling and reversing each item in coupling map), but useful to run anyway
        '''
    ...
    def get_best_node(self):
        '''
        First node with the most connections; Does not yet sort based on error, but that is probably not too useful
        '''
...
#                 newcxlengthdict[(a[1],a[0])] = cxlengthdict[a]
    ...
    def get_tier_dict(self):      
        ...
        while True: 
            ...
            for parent in parentlist:
                ...
                if self.indicator:
                    children = self.child_sorter(children,parent) #remove if dont want error mitigation
                totalchildren += children #new
    ...
    def get_ghz_layout(self,n,transpiled=True,barriered=True): #n qubit system
        '''
        Feeds the Tier Dict of the backend to create a basic qiskit GHZ circuit with no mesaurement;
        ...
        for a in tierdict.keys():
            for aa in tierdict[a]:
                ...
                if aa[1] not in trashlist:
                    ...
                    pass
                circ.cx(q[trashlist.index(aa[0])],q[trashlist.index(aa[1])])
        ...
        if transpiled:
            circ = qiskit.compiler.transpile(circ,backend=self.backend,initial_layout=initial_layout)
    ...
    def get_ghz_mqc(self,n,delta,extent = 'full'):
        ...
        rotate.x(q)
        rotate.barrier()
        rotate = qiskit.compiler.transpile(rotate,backend=self.backend,initial_layout=initial_layout)
...
#         meas.measure(q,cla)
        ...
        meas = qiskit.compiler.transpile(meas,backend=self.backend,initial_layout=initial_layout)
